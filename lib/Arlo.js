...
class Arlo extends EventEmitter {
    ...
    async loginAndKeepAlive(user, password, callback, heartbeat = 30000) {
        ...
        let found = false;
        this.addListener(Constants.EVENT_MESSAGE, () => msLastResponse = new Date());
        this.addListener(Constants.EVENT_GOT_DEVICES,() => found=true);
        while (this.keepAlive) {
            if (new Date() - msLastResponse > heartbeat)
                this.login(user, password, callback);
            else if (found)
                for(var name in this.devices)
                    if ( [Constants.TYPE_BASESTATION, Constants.TYPE_ARLOQS, Constants.TYPE_ARLOQ].includes(this.devices[name].device.deviceType) )
                        this.devices[name].subscribe();
